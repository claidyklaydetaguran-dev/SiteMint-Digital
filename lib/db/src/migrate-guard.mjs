...
//
// It also decides WHICH database is being migrated, explicitly. The caller names
// a target, states the identity it expects to reach, and the guard verifies that
// against the live connection before taking the lock — then hands drizzle-kit
// that same connection rather than letting it resolve an ambient DATABASE_URL.
// See ./db-target.mjs for why a named-but-unverified target is not enough.
//
//   node ./src/migrate-guard.mjs scheduling \
//     --target dev --expect-db <name> --expect-fingerprint <hex12>
//
// Run `node ./src/db-identity.mjs --target <name>` first to read those values.
...
export async function readState(client, domains) {
...
/**
 * drizzle-kit reads its own connection from the config's `dbCredentials`, which
 * resolve `process.env.DATABASE_URL`. The guard resolves the URL from the NAMED
 * target instead, so the child is handed that exact connection rather than
 * whatever the ambient environment happened to hold. Without this, `--target
 * prod` would verify the deployment database and then migrate development.
 */
function runDrizzleKit(domain, databaseUrl) {
...
async function main() {
  ...
  // Which database, stated explicitly and verified below — before the lock,
  // before the journal read, and before any child process.
  const target = resolveDbTarget({ argv: process.argv.slice(3), env: process.env });
  if (!target.ok) {
    console.error(`migrate guard — ${target.message}`);
    process.exit(1);
  }
  ...
  console.error(renderTargetBanner({ ...target, identity }));
  if (!verified.ok) {
    console.error(`migrate guard — ${verified.message}`);
    await client.end();
    process.exit(1);
  }
  ...
  try {
    ...
    exitCode = await runDrizzleKit(domain, databaseUrl);
  } finally {
    try {
