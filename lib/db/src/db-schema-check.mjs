...
const EXPECTED = [
  ...
  {
    tag: "voice 0011_fat_firebrand",
    tables: ["voice_business_profiles"],
    columns: [
      ["voice_business_profiles", "primary_contact_name"],
      ["voice_business_profiles", "primary_contact_email"],
      ["voice_business_profiles", "default_location"],
    ],
    indexes: ["uq_voice_business_profiles_firm_id"],
    constraints: [
      "ck_voice_business_profiles_contact_name",
      "ck_voice_business_profiles_contact_email",
      "ck_voice_business_profiles_location",
      "voice_business_profiles_firm_id_intake_firms_id_fk",
    ],
  },
  {
    tag: "voice 0014_member_accounts_policy_contacts",
    tables: ["voice_policy_acceptances"],
    columns: [
      ["voice_policy_acceptances", "version"],
      ["voice_policy_acceptances", "accepted_at"],
      ["voice_firm_members", "password_hash"],
      ["voice_firm_members", "invite_token_hash"],
      ["voice_contacts", "origin"],
      ["voice_contacts", "email"],
      ["voice_contacts", "notes"],
    ],
    indexes: ["uq_voice_policy_acceptances_firm_policy_version", "ix_voice_firm_members_email_status"],
    constraints: [
      "ck_voice_policy_acceptances_policy",
      "ck_voice_policy_acceptances_version",
      "ck_voice_contacts_origin",
      "ck_voice_firm_members_invite_hash_shape",
      "voice_policy_acceptances_firm_id_intake_firms_id_fk",
    ],
  },
  {
    tag: "voice 0013_support_requests",
    tables: ["voice_support_requests", "voice_support_messages"],
    columns: [
      ["voice_support_requests", "status"],
      ["voice_support_requests", "requested_by_email"],
      ["voice_support_requests", "operator_notified_at"],
      ["voice_support_messages", "author"],
    ],
    indexes: ["ix_voice_support_requests_firm_status", "ix_voice_support_messages_request_created"],
    constraints: [
      "ck_voice_support_requests_status",
      "ck_voice_support_requests_closed_is_stamped",
      "ck_voice_support_messages_author",
      "voice_support_requests_firm_id_intake_firms_id_fk",
      "voice_support_messages_request_id_voice_support_requests_id_fk",
    ],
  },
  {
    tag: "voice 0012_notification_delivery_evidence",
    tables: [],
    columns: [
      ["voice_notifications", "first_attempt_at"],
      ["voice_notifications", "outcome_uncertain_at"],
      ["voice_notifications", "delivery_status"],
      ["voice_notifications", "delivery_event_at"],
      ["voice_usage_ledger", "channel"],
    ],
    indexes: ["ix_voice_notifications_provider_message"],
    constraints: [
      "ck_voice_notifications_delivery_status",
      "ck_voice_notifications_delivery_only_when_accepted",
      "ck_voice_usage_ledger_channel",
    ],
  },
  {
    // Push-managed, not a versioned migration — but operator sign-in that
    // survives a restart depends on it, so its absence must be visible before
    // anyone relies on the cookie session in a given environment.
    tag: "operator sessions (push: crm_admin_sessions)",
    tables: ["crm_admin_sessions", "crm_admin_audit_log"],
    columns: [
      ["crm_admin_sessions", "token_hash"],
      ["crm_admin_sessions", "expires_at"],
      ["crm_admin_sessions", "revoked_at"],
    ],
    indexes: [],
    constraints: [],
  },
