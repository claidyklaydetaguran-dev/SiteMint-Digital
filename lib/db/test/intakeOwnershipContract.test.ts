...
async function main(): Promise<void> {
  ...
  // ── candidate artifacts exist with exactly the expected shape (A2) ─────────
  // A2 generated the single initial migration from the intakeVersioned schema.
  // These assertions replace the A1 "directory must be empty" check.
  const candidateDir = path.join(dbRoot, "drizzle-intake-candidate");
  assert(existsSync(candidateDir), "drizzle-intake-candidate/ directory exists (generated by A2)");
  if (existsSync(candidateDir)) {
    const sqlFiles = readdirSync(candidateDir)
      .filter((f) => f.endsWith(".sql"))
      .sort();
    assert(
      sqlFiles.length === 1 && sqlFiles[0] === "0000_talented_virginia_dare.sql",
      "drizzle-intake-candidate/ contains exactly one SQL file: 0000_talented_virginia_dare.sql",
    );
    const metaDir = path.join(candidateDir, "meta");
    assert(existsSync(metaDir), "drizzle-intake-candidate/meta/ directory exists");
    if (existsSync(metaDir)) {
      const metaFiles = readdirSync(metaDir).sort();
      assert(
        JSON.stringify(metaFiles) === JSON.stringify(["0000_snapshot.json", "_journal.json"]),
        "drizzle-intake-candidate/meta/ contains exactly _journal.json and 0000_snapshot.json",
      );
    }
  }
