...
//
//   Honesty       A recorded reply is not a sent reply. Nothing in this module
//                 emails or texts anybody; every customer-visible message says
//                 so in its response, and the one place delivery would hook in
//                 is marked. Counts returned alongside a list are computed from
//                 the same filters as the list, so a queue badge and the queue
//                 cannot disagree.
//
...
/**
 * A thread entry as staff see it — author included, visibility stated.
 *
 ...
 *
 * Nothing is sent. This records what was said; delivery is a separate concern
 * and is not wired here (see the seam below).
 */
router.post("/crm/support/tickets/:id/messages", requireCrmAuth("support.write"), async (req: Request, res: Response) => {
  ...
  // ── Delivery seam ─────────────────────────────────────────────────────────
  // A customer-visible message is RECORDED here, not sent. When the shared
  // delivery helper lands (artifacts/api-server/src/lib/delivery.ts, owned by
  // the integration lead), this is the single place it hooks in — one call,
  // guarded by `visibility === "customer"`, with the send result written back
  // onto this message row. Until then the response says plainly that nothing
  // left the building, because a CRM that implies it emailed somebody when it
  // did not is worse than one that cannot email at all.
  ...
  if (visibility === "customer") {
    await db.insert(crmActivities).values({
      leadId: ticket.leadId,
      type: "support_reply_recorded",
      title: `Reply recorded on support ticket ${supportTicketReference(id)}`,
      description: "Recorded in the CRM. Not sent — no delivery is wired to Support yet.",
      createdBy: me.label,
    });
  }
  ...
  res.status(201).json({
    ...
    delivery: visibility === "customer"
      ? {
          sent: false,
          note: "Recorded on the ticket. Nothing was emailed or texted — Support has no delivery wired to it yet. Send it from Communications if the client needs it now.",
        }
      : {
          sent: false,
          note: "Internal note. It stays in the CRM and never appears on a customer-facing surface.",
        },
  });
});
