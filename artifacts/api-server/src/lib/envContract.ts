...
const FLAGS: Array<[string, string, string]> = [
  ...
  ["VOICE_SERVICE_ACCESS_REQUIRED", "J7", "Publishing, sync and browser test calls require an activated plan (a subscription in active or grace whose plan is in VOICE_PLAN_CATALOG_JSON)"],
...
const SECRETS: Array<[string, string, string]> = [
  ...
  ["RESEND_API_KEY", "P7", "Resend key for alerts/digest/account/CRM email. Sending-only access is enough and is recommended"],
...
const CONFIGS: Array<[string, string, string]> = [
  ...
  ["VOICE_SMS_PUBLIC_ORIGIN", "J4", "Public https origin Twilio reaches this API on; used for signature verification and the delivery-status callback (falls back to the VOICE_SERVER_URL origin)"],
  ["VOICE_SMS_DAILY_CAP_PER_FIRM", "J4", "Spending cap: texts one business may send per UTC day (default 20, 1-500); over the cap a text is closed as failed, never held"],
  ["VOICE_SMS_DAILY_CAP_TOTAL", "J4", "Spending cap: texts the platform may send per UTC day (default 100, 1-5000)"],
