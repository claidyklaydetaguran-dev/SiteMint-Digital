...
export type RemoteOutcome = "deleted" | "already_absent";
...
export async function cleanupStagingAssistant(
  ...
  try {
    ...
    if (err instanceof VoiceProviderError) {
      // NOT_FOUND is authoritative: VapiVoiceProvider maps it from an HTTP 404
      // on DELETE /assistant/{id} and from nothing else, so the resource is
      // genuinely absent rather than merely unconfirmed. This is the only
      // branch that treats a failure as a successful cleanup, and it never
      // fabricates a provider confirmation.
      if (err.code === "NOT_FOUND") {
        remote = "already_absent";
      } else if (DEFINITIVE_FAILURE_CODES.has(err.code)) {
  ...
  if (!reconciled) {
    // Remote deletion really did happen; only the local write failed or matched
    // zero rows (the row changed underneath us). Report both halves honestly and
    // keep the identity a later run needs — that run will observe the resource
    // already absent and finish the reconciliation.
...
export function parseCleanupArgs(argv: readonly string[]): CleanupCliArgs {
  ...
  for (const raw of argv) {
    ...
    if (key === "--firm-id") args.firmId = Number(value);
    else if (key === "--assistant-id") args.assistantId = Number(value);
...
export function formatCleanupResult(result: CleanupResult): string {
  ...
  switch (result.status) {
    ...
    case "cleaned":
      lines.push(
        result.remote === "deleted"
          ? "CLEANED — the remote assistant was deleted and local state was reconciled."
          : "CLEANED — the remote assistant was already absent and local state was reconciled.",
      );
      push("firmId", result.firmId);
      push("assistantId", result.assistantId);
      push("providerAssistantId", result.providerAssistantId);
    ...
    case "provider_failed":
      ...
      lines.push("  Nothing was retried. Re-run deliberately after resolving the cause.");
      break;
    case "uncertain":
      ...
      lines.push("  Local state is unchanged and still records the provider link.");
      lines.push("  Nothing was retried. Verify the resource in the provider account before acting.");
      break;
    case "local_reconcile_failed":
      lines.push("PARTIAL — the remote assistant is gone but local state was NOT reconciled.");
      push("firmId", result.firmId);
      push("assistantId", result.assistantId);
      push("providerAssistantId", result.providerAssistantId);
      push("remote", result.remote);
      lines.push("  Re-run this command to finish; it will observe the resource already absent.");
      break;
  }
