 ...
 * with a tab-local summary, reviewed on the Start page. Previously carried in the
 * URL; now copied or emailed explicitly before submission. The brief's "why
 * now"/"desired outcome" fields can receive the copied summary. Nothing is submitted here,
 * and the discovery API payload remains unchanged.
 */

import { useMemo, useState } from "react";
import { Link } from "wouter";
import { ROUTES } from "@/lib/routes";
import { SCOPE_DRAFT_KEY } from "@/components/mint/InquiryContext";
...
const SCOPE_ADD_ONS = [
  ...
  { id: "custom-app", label: "A custom web application" },
  {
    ...
    label: "Growth infrastructure (tracking, pixels, consent)",
  },
  {
    id: "advertising",
    label: "Advertising services (separately scoped, recurring)",
  },
...
function TierCard({ tier }: { tier: PricingTierV5 }) {
  return (
    <div
      className={`v3-card reveal-scale-settle${tier.recommended ? " v3-card--hover" : ""}`}
      style={{ padding: "1.75rem", display: "grid", gap: "0.75rem" }}
    >
      {tier.recommended && (
        <span
          className="v3-eyebrow"
          style={{ color: "var(--sm-mint-700, #0B7487)" }}
        >
          Most common starting point
        </span>
      )}
      <h2 className="v3-h2" style={{ fontSize: "1.3rem" }}>
        {tier.name}
      </h2>
      <p
        style={{
          fontFamily: "var(--sm-font-display, inherit)",
          fontSize: "1.6rem",
          color: "var(--sm-mint-700, #0B7487)",
          margin: 0,
        }}
      >
        {tier.priceFrom}
      ...
      <p
        className="v3-body"
        style={{ fontSize: "0.9rem", color: "var(--sm-text-muted, #4A6472)" }}
      >
      ...
      {tier.honestyNote && (
        <p
          style={{ fontSize: "0.8rem", color: "var(--sm-text-muted, #4A6472)" }}
        >
          {tier.honestyNote}
        </p>
      )}
...
function ScopeConfigurator() {
  ...
  const summary = useMemo(() => {
    const lines = [
      ...
      addOns.size > 0
        ? `Additional scope: ${SCOPE_ADD_ONS.filter((a) => addOns.has(a.id))
            .map((a) => a.label)
            .join(", ")}`
  ...
  const startHref = `${ROUTES.start}?scope=draft`;

  return (
    <div
      className="v3-card"
      style={{ padding: "1.75rem", display: "grid", gap: "1.25rem" }}
    >
      <div>
        ...
        <p className="v3-body">
          Pick a starting point and any additional scope you already know about.
          This composes a summary you can bring straight into your discovery
          brief — it doesn't submit anything on its own.
        </p>
      </div>

      <fieldset style={{ border: "none", padding: 0, margin: 0 }}>
        <legend className="v3-eyebrow" style={{ marginBottom: "0.5rem" }}>
          Starting point
        </legend>
      ...
      <Link
        ...
        onClick={(event) => {
          try {
            sessionStorage.setItem(SCOPE_DRAFT_KEY, summary);
      ...
      <p className="v3-body" role="status">
        {storageError
          ? "Your browser couldn't save this summary. Copy the text above, then use Contact SiteMint in the footer."
          : "Kept only in this browser tab until you close it. Not sent to SiteMint."}
      </p>
    </div>
  );
}
    ...
    <div className="v3-services-hub sm-v5page">
      <section style={{ backgroundImage: `linear-gradient(90deg, #f8fbf7f5, #f8fbf7b0 48%, #f8fbf720), url(${mintHeroBackground})` }} className="v3m-page-hero mint-image-hero" data-tone="porcelain">
        ...
        >
          ...
          <h1 className="v3-display">
            Three starting points. Every system is scoped.
          </h1>
      ...
      <section className="v3-section" data-tone="porcelain">
        <div className="v3-container v3-reveal" ref={reveal}>
          ...
          <p
            className="v3-body reveal-fade-up"
            style={{ marginTop: "0.5rem", maxWidth: "62ch" }}
          >
            Meta Ads and Google Ads management — campaign discovery and
            strategy, offer/audience/funnel planning, account and campaign
            setup, landing-page and creative coordination, pixels and conversion
            events, launch, and ongoing monitoring with plain reporting. Growth
            infrastructure (tracking, pixels, consent, landing pages) can be set
            up on its own, independent of whether you ever run paid media with
            us.
          </p>
      ...
      <section className="v3-section" data-tone="white">
        <div className="v3-container v3-reveal" ref={reveal}>
          ...
          <p className="v3-body reveal-fade-up" style={{ marginTop: "0.5rem" }}>
            ...
            starting price, and every one of them gets discussed plainly during
            discovery, before we commit to a number.
          </p>
          <div className="sm-price-factors">
            {PRICE_FACTORS.map((factor) => (
              <div
                className="sm-price-factor reveal-scale-settle"
                key={factor.title}
              >
      ...
      <section className="v3-section v3m-cta" data-tone="ink">
        <div className="v3-container v3m-cta__inner v3-reveal" ref={reveal}>
          <h2 className="v3-display reveal-clip">
          ...
          <div className="v3m-cta__actions">
            <Link
              href={ROUTES.start}
              className="v3-btn v3-btn--primary reveal-fade-up"
            >
              Plan My Project
            </Link>
          </div>
