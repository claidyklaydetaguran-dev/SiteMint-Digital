...
export default function PortalSignIn() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const data = await portalFetch<{ csrfToken: string }>("/api/portal/login", {
        method: "POST", body: { email, password },
      });
      setPortalCsrf(data.csrfToken);
      navigate("/portal");
    } catch (err) {
      setError(err instanceof PortalError ? err.message : "Something went wrong at our end.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-4 py-10">
        ...
        <h1 className="mt-6 text-center text-xl font-semibold tracking-tight">Your project, in one place</h1>
        <p className="mt-2 text-center text-sm text-muted-foreground">
          Sign in to see your projects, documents, payments and support requests.
        </p>
        ...
        <form onSubmit={submit} className="mt-8 space-y-4" noValidate>
          <div className="space-y-2">
            ...
            <Input
              id="portal-email" type="email" autoComplete="username" inputMode="email"
              className="min-h-11" value={email} required
          ...
          <div className="space-y-2">
            ...
            <Input
              id="portal-password" type="password" autoComplete="current-password"
              className="min-h-11" value={password} required
        ...
        <p className="mt-8 text-center text-sm text-muted-foreground">
          Access is set up by your SiteMint contact. If you have not had an invitation
          yet, email us and we will send one.
        </p>
      </div>
    </div>
  );
