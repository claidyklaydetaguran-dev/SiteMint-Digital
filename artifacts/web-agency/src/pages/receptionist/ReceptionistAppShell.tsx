...
export function useReceptionistShell() {
  return null; // placeholder — extend if child pages need firm data
}
...
export default function ReceptionistAppShell({ children }: ReceptionistAppShellProps) {
  ...
  const logout = async () => {
    await fetch("/api/receptionist/auth/logout", {
      method: "POST",
      credentials: "include",
    });
    navigate("/app/login");
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "#f8fafc" }}>
      ...
      <div style={{
        ...
        {pct >= 80 && <AlertCircle size={14} style={{ flexShrink: 0 }} />}
        <span>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        {/* Desktop sidebar */}
        <div className="hidden md:flex" style={{ flexDirection: "column", height: "calc(100vh - 38px)", position: "sticky", top: 0 }}>
          <Sidebar />
        </div>

        {/* Mobile top bar */}
        <div className="md:hidden" style={{
          position: "fixed", top: 38, left: 0, right: 0, zIndex: 40,
          background: "#fff",
        ...
        <AnimatePresence>
          {sideOpen && (
            <motion.div
              ...
              className="md:hidden"
              style={{
                position: "fixed", top: 86, left: 0, right: 0, bottom: 0,
        ...
        <main style={{
          ...
          overflowY: "auto",
          paddingTop: 0,
        }} className="mt-0 md:mt-0 pt-[50px] md:pt-0">
          {children}
        </main>
      </div>
    </div>
