...
export default function CrmDocuments() {
  ...
  //
  // Projects and leads are the two that matter day to day. Both lists are
  // already permission-checked by their own routes.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const [projRes, leadRes] = await Promise.all([
        adminFetch("/api/crm/projects").catch(() => null),
      ...
      if (projRes?.ok) {
        ...
        for (const p of data.projects ?? []) {
          opts.push({ kind: "project", id: p.id, label: p.name, sub: p.stage ?? undefined });
        }
      }
      if (leadRes?.ok) {
        const data = await leadRes.json().catch(() => ({}));
        for (const l of data.leads ?? []) {
          opts.push({ kind: "lead", id: l.id, label: l.name, sub: l.company ?? undefined });
        }
      }
      if (opts.length === 0) {
        setSubjectsError("No projects or contacts were found, so there is nothing to attach a file to yet.");
      }
      setSubjects(opts);
      setSubject(prev => prev ?? opts[0] ?? null);
    })();
  ...
  const loadRequests = useCallback(async () => {
    ...
    if (!res.ok) {
      ...
      throw new Error(body.error || `Document requests could not be loaded (${res.status}).`);
    }
  ...
  const loadDocuments = useCallback(async () => {
    if (!subject) { setDocuments([]); setSupersededCount(0); return; }
    const params = new URLSearchParams({
      entityType: subject.kind,
      entityId: String(subject.id),
      includeSuperseded: showSuperseded ? "true" : "false",
    });
    const res = await adminFetch(`/api/crm/documents?${params.toString()}`);
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      throw new Error(body.error || `Files could not be loaded (${res.status}).`);
    }
    const data = await res.json();
  ...
  const openRequests = useMemo(() => requests.filter(r => r.status === "pending"), [requests]);
  const overdueRequests = useMemo(() => openRequests.filter(r => r.overdue), [openRequests]);

  // Requests for whichever record is on screen, so the two halves line up.
  const subjectRequests = useMemo(
    () => subject ? requests.filter(r => r.entityType === subject.kind && r.entityId === subject.id) : [],
    [requests, subject],
  );
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      const res = await adminFetch("/api/crm/documents", {
        method: "POST",
      ...
      if (!res.ok) {
        setError(data.error || `The upload was refused (${res.status}).`);
        return;
      }
      setNotice(
  ...
  // without an Authorization header. Fetch the bytes, then hand them over.
  async function download(doc: DocumentRow) {
    const res = await adminFetch(`/api/crm/documents/${doc.id}/download`);
    if (!res.ok) { setError(`"${doc.filename}" could not be downloaded.`); return; }
  ...
  async function ask() {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { setError(data.error || `The request could not be recorded (${res.status}).`); return; }
  ...
  async function closeRequest(req: DocumentRequest, status: "received" | "cancelled") {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        setError(data.error || `That request could not be updated (${res.status}).`);
        return;
      }
  ...
  async function openShares(doc: DocumentRow) {
    ...
    const res = await adminFetch(`/api/crm/documents/${doc.id}/shares`);
  ...
  async function createShare() {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { setError(data.error || `The link could not be created (${res.status}).`); return; }
  ...
  async function openSharesList(attachmentId: number) {
    const res = await adminFetch(`/api/crm/documents/${attachmentId}/shares`);
    setShares(res.ok ? (await res.json()).shares ?? [] : []);
  }
      ...
      <div className="p-4 sm:p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <div className="bg-background border border-border rounded-xl px-4 py-3">
            ...
            <p className="text-2xl font-bold text-foreground mt-1">{openRequests.length}</p>
          </div>
          <div className="bg-background border border-border rounded-xl px-4 py-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Past due</p>
            <p className={`text-2xl font-bold mt-1 ${overdueRequests.length > 0 ? "text-red-600" : "text-foreground"}`}>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[320px_minmax(0,1fr)] gap-4">
          ...
          <div className="bg-background border border-border rounded-xl overflow-hidden">
            ...
            {subjectsError ? (
              <p className="px-4 py-6 text-xs text-muted-foreground">{subjectsError}</p>
            ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-background border border-border rounded-xl overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
                <h2 className="text-sm font-bold text-foreground">Waiting on the client</h2>
            ...
            {/* Everything outstanding, across records */}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-foreground/40 p-0 sm:p-4"
          ...
          <div className="bg-background w-full sm:max-w-lg sm:rounded-xl rounded-t-xl border border-border max-h-[92vh] overflow-y-auto"
            ...
            <div className="sticky top-0 bg-background flex items-center gap-2 px-4 py-3 border-b border-border">
              <h3 className="text-sm font-bold text-foreground truncate">Share "{shareFor.filename}"</h3>
              <button onClick={() => setShareFor(null)} className="ml-auto text-muted-foreground hover:text-foreground">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-4 space-y-3">
              {freshLink && (
                <div className="rounded-lg border border-teal-200 bg-teal-50 px-3 py-2.5 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <code className="flex-1 text-[11px] bg-background border border-teal-200 rounded px-2 py-1.5 truncate">
                      {freshLink}
                    </code>
