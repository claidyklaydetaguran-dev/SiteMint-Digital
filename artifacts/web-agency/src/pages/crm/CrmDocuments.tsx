...
export default function CrmDocuments() {
  ...
  async function remove(doc: DocumentRow) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(`"${doc.filename}" was not deleted. ${await responseFailureReason(res)}`);
        return;
      }
      setNotice(`"${doc.filename}" was deleted.`);
      await refresh(true);
    } catch {
      setError(`"${doc.filename}" was not deleted. ${failureReason(null)}`);
    } finally {
      setBusy(false);
    }
  }
  ...
  async function revokeShare(share: ShareRow) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(`That link was not revoked. ${await responseFailureReason(res)}`);
        return;
      }
      ...
      setError(`That link was not revoked. ${failureReason(null)}`);
    } finally {
      setBusy(false);
    }
  }
