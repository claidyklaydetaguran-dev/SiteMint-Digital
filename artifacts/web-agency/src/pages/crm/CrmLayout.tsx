...
// ── Sidebar nav item ───────────────────────────────────────────────────────────
// Light Mint Clarity sidebar (styles/crm-workspace.css, `crm-nav*`). A real link
// element carries the destination and `aria-current`; the previous markup
// nested a <button> inside the link, which is invalid interactive nesting.
function SideNavItem({ item, isActive, onNavigate }: {
  ...
  if (item.comingSoon) {
    return (
      <div className="crm-navitem" data-disabled="true" aria-disabled="true">
        <Icon className="crm-navitem__icon" aria-hidden="true" />
        <span className="crm-navitem__label">{item.label}</span>
        <span className="crm-navitem__soon">Soon</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {item.href === "/admin/dashboard" && (
        <ExternalLink className="crm-navitem__ext" aria-hidden="true" />
      )}
    </Link>
  );
}
...
function SidebarContent({
  ...
  return (
    <nav data-testid="crm-sidebar-nav" aria-label="CRM" className="crm-sidenav flex flex-col h-full overflow-x-hidden">
      {NAV_GROUPS.map(group => {
        const GroupIcon = group.icon;
        const expanded = expandedGroups.has(group.id);
        const hasActive = group.items.some(item => isItemActive(item));
        const listId = `crm-navgroup-${group.id}`;

        return (
          <div key={group.id} className="crm-navgroup" data-active={hasActive ? "true" : "false"}>
            <button
              type="button"
              onClick={() => toggleGroup(group.id)}
              className="crm-navgroup__toggle"
              aria-expanded={expanded}
              aria-controls={listId}
            >
              <GroupIcon className="crm-navgroup__icon" aria-hidden="true" />
              <span className="crm-navgroup__label">{group.label}</span>
              <ChevronDown className="crm-navgroup__chevron" aria-hidden="true" />
            </button>

            {expanded && (
              <div id={listId} className="crm-navgroup__items">
