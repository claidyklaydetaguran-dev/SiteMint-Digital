...
- Earlier shell checks used incorrect Google variable names and a potentially stale shell environment. They do not establish credential absence. The implementation reads GOOGLE_OAUTH_CLIENT_ID / GOOGLE_OAUTH_CLIENT_SECRET / GOOGLE_OAUTH_REDIRECT_URI / CALENDAR_CONNECT_ENABLED. CRM_BASE_URL was a development hostname; provider routing still requires reconciliation.
...
## Production setup update — supersedes earlier Google access findings

- Business owner signed into the new `sitemint-production` Google project. Calendar API is enabled. With explicit owner approval, the three scopes used by the implementation were saved: calendar.freebusy, calendar.events, calendar.calendarlist.readonly.
- All three owner-approved test accounts were saved and independently confirmed after reload: 3 test users. The app remains External / Testing. Public verification is not submitted or approved; this is not general customer availability.
- Production web OAuth client has only the live-domain callback `/api/receptionist/calendar/google/callback`. Credentials are saved in Web Asset Builder Secrets, never in source. Production deployment settings include the three OAuth variables and CALENDAR_CONNECT_ENABLED=true. Backend republish started; live connection acceptance remains pending. Development database copying and Stripe synchronization were both off.
- Backend workspace is clean at Replit's `3a96bfb` publishing snapshot dated 2026-09-22. No backend source changes or migrations were made in this configuration pass.
- Public source `ac5449b` packaged in `d406c1e`; canonical Git-byte checksums passed in Replit. Only `mkt` replaced, with rollback at `mkt.rollback-before-ac5449b`. Marketing deployment `a4f67855-c035-4320-b81a-7de9595f6574` is live: public `/release.json` confirms `ac5449b`. Homepage challenge selector and journey accordion work in the live browser. Services, receptionist, work, pricing, about, discovery, privacy and terms return 200 with one H1 and a canonical link. Robots and sitemap return 200. API readiness, CRM and client-portal entry routes return 200; these checks do not certify authenticated workflows.
- Receptionist production session requires user sign-in; staging identity does not prove a production account exists. Calendar connection, booking, CRM application-path delivery and phone callback acceptance remain open. No calls, SMS or payments were performed.

## Registration and recovery incident — 2026-09-23

- Production `/api/receptionist/auth/register` and `/api/receptionist/account/password-reset/request` both returned 503 with their exact disabled-gate messages. This was configuration, not evidence of a bad password. Do not direct the owner to retry disabled forms.
- Owner explicitly approved enabling public account registration and password recovery. Registration creates a customer business and session, not an operator role, phone purchase or paid subscription.
- Read-only production queue audit: zero voice notification rows; signup jobs contained one completed CRM link and one permanently failed verification email. No pending email jobs would be released by enabling the transport. Existing failed jobs were not replayed.
- Published production PUBLIC_REGISTRATION_ENABLED=true, PASSWORD_RESET_REQUESTS_ENABLED=true, VOICE_ALERTS_ENABLED=true, VOICE_ALERTS_FROM=noreply@sitemintdigital.com, VOICE_ALERTS_TO=the confirmed owner mailbox, and VOICE_DASHBOARD_BASE_URL=https://sitemintdigital.com. No credentials recorded here. Database copying and Stripe synchronization remained off. Replit build f0235c2f-125f-4a5f-b0d9-977668796f25 published successfully. Live empty-body registration probe now returns 400 required-fields validation instead of disabled 503; empty-body reset probe returns 200 accepted; readiness returns 200. These probes create no accounts or emails and do not prove inbox delivery. Owner signup and its verification email are the next acceptance steps.
- Targeted registration guards, password-reset token/security behavior and account-email link tests: 3 files, 43 tests passed. These mocked tests do not certify production account creation or inbox delivery. Owner must enter and submit their own new password.

## Mint Clarity release and calendar acceptance
Public ba22195 and private workspace presentation/contrast fef1bc22 are now published. Approved production calendar create/cancel passed, with no guests or notifications. Confirmed US Pacific weekday hours and 30-minute Discovery consultation are saved. See 2026-09-23-mint-refinement-handoff.md for exact evidence and remaining pilot boundaries. This supersedes earlier pending visual-release and calendar-acceptance status; it does not certify phone, SMS, CRM email or billing.
