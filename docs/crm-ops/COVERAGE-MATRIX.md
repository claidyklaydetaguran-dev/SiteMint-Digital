...
| 3 | Pipeline, deals, forecasting | **Partial (improved M2)** | Kanban and deals pre-existing. Transactions rebuilt: one joined, filtered, server-paged query with whole-filtered-set totals (the N+1 fan-out is gone). Stage-weighted forecast with stated assumptions. Still missing: probability per deal, won/lost reasons, multi-currency. |
| 4 | Email / SMS / calls | **Partial (improved M3)** | Reads and sends exist via `phone.ts`/Resend on staff sessions with per-route permissions. M3: the inbox has real per-person unread state (`crm_thread_reads`) that survives a reload and does not leak between people, plus a readers list so you can see a colleague already opened a conversation; email CC/BCC are actually sent (they were posted and ignored); the email body is kept instead of discarded; email-activity and template routes are gated on `communications.read`/`.send` rather than bare authentication. Still missing: **no inbound email at all**, no conversation table (threads are assembled in memory from the latest 200 messages), no sender recorded on SMS or calls (blocked on `phone.ts`, which is protected), and two near-duplicate inbox screens. |
...
| 12 | Unified interaction / purchase / note history | **Partial (improved M3)** | `crm_activities` timeline plus three other event tables; still fragmented and lead-scoped. M3 fixed the attribution: every entry defaulted to the literal `created_by = "admin"` and nothing overrode it, so the timeline could not say which of the three owners did anything. It now names the signed-in person at all thirteen write sites in `crm.ts`. SMS and call entries are still unattributed — that code is in the protected `phone.ts`. |
