...
| 9 | Documents / proposals / invoices | **Partial** | Proposal + SOW HTML persist on the record; **invented budget/timeline/score removed (M2)** — an unsupplied field stays unspecified and no longer feeds scoring or package choice. `crm_attachments` table exists (versioning, content hash, private storage key) but upload/download endpoints and UI are not built. No signing integration. |
