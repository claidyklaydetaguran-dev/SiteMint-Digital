...
| 10 | Workflow automation & approvals | 9 / 10 | **90%** | L, T | 19 + 5 + 10 tests; three loop brakes; approvals by identity; durable event log; time triggers now fire with corrected overdue semantics | No UI surfaces failed automation events — visible only by querying the table |
