...
| 26B — SiteMint personas + topic taxonomy | ~~Add persona + topic constants (data only, no schema) used to label/seed campaigns~~ **DONE** | `lib/campaignTaxonomy.ts`, `CrmCampaigns.tsx` | No | Low | Implemented — taxonomy constants live in `lib/campaignTaxonomy.ts` |
...
| 26D — Stop-on-reply hardening | ~~Add reply-detection for email replies (not just complaints) + audit coverage~~ **DONE** | `lib/sequenceReply.ts`, scheduler | No | Med | Implemented — reply-detection + audit coverage live in `lib/sequenceReply.ts` and scheduler |
